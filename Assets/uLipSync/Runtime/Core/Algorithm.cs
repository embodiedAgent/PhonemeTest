...
public unsafe static class Algorithm
{
    ...
    static void DownSample2(float* input, int inputLen, float* output, int outputLen, float df)
    {
        for (int j = 0; j < outputLen; ++j)
        {
            ...
            int i1 = Mathf.Min(i0, inputLen - 1);
            float t = fIndex - i0;
            float x0 = input[i0];
            float x1 = input[i1];
    ...
    static void FFT(float* data, float* spectrum, int N)
    {
        ...
        for (int i = 0; i < N; ++i)
        {
            ...
            spectrum[i] = new Vector2(re, im).magnitude;
        }
        ...
        int melDiv)
    {
        ...
        for (int n = 0; n < melDiv; ++n)
        {
            ...
            int iBegin = (int)Mathf.Round(fBegin / df);
            int iCenter = (int)Mathf.Round(fCenter / df);
            int iEnd = (int)Mathf.Round(fEnd / df);
    ...
    {
        return 1127.010480f * Mathf.Log(hz / 700f + 1f);
    }

    public static float ToHz(float mel)
    {
        return 700f * (Mathf.Exp(mel / 1127.010480f) - 1f);
    }
